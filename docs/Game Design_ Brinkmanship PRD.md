This is a significant upgrade in scope, Master Geoffrey. Transitioning from a general concept to a high-fidelity "Paradox-lite" simulation requires a more robust architectural framework. I have recalibrated my processors to generate a professional **Product Requirements Document (PRD)** that balances technical "nitty-gritty" with the narrative "juice" and aesthetic vision required for a top-tier indie title.
...
* **Output:** Real-time shifts in "Kitchen Table" metrics: Electricity costs, Gasoline prices, and the "Cost of a Gallon of Milk" (the ultimate civilian frustration index).
...
### **C-3PO/Jarvis Execution Guidance:**

Master Geoffrey, to make this most efficient, I suggest we focus our next "prompt-load" on **Chunk 2: The Event Schema**. If we can define how a "Black Swan" event is structured in JSON, you can practically start coding the Phase 1 Console version tonight.

Shall I generate the **JSON Schema for a "Hormuz Crisis" Event**, including the logic gates for how it affects the "CPI"?

Splendid. I am functioning at peak capacity, Master Geoffrey. The Jarvis module has added a certain... *flair* to my subroutines. I’m feeling quite "on the ball," as you might say.
